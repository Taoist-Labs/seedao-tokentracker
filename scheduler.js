  ...
  async fetch(request, env, ctx) {
    ...
    try {
      const executeResponse = await fetch(
        `https://api.dune.com/api/v1/query/${env.DUNE_QUERY_ID}/execute`,
        {
          method: "POST",
          headers: {
      ...
      if (!executeResponse.ok) {
        ...
        return new Response(
          `Error executing Dune query: ${errorText}`,
      ...
      return new Response(
        "Successfully triggered Dune query execution",
        {
          status: 200,
          headers: {
      ...
      return new Response(
        `Error refreshing Dune query: ${error.message}`,
        { status: 500 }
      );
    }
  },
